/**
 * Temporary Bootstrap Service
 * 
 * This module provides a one-time bootstrap for empty databases.
 * It's designed to be temporary and easy to remove.
 * 
 * Usage: Add as middleware in index.js, then remove later when no longer needed.
 */
...
// In-memory lock for concurrent request safety
...
async function isDatabaseEmpty() {
  try {
    ...
    return false; // Assume not empty on error for safety
  }
}
...
async function createSchool(schoolIndex, hashedPassword, superAdminId, now) {
  ...
  const school = new School({
    ...
    isVerified: true,
    tags: ['bootstrap', 'demo'],
    notes: 'Temporary bootstrap-generated school',
...
async function bootstrapDatabase() {
  ...
  try {
    ...
    // Check if bootstrap already completed (safety)
    const marker = await BootstrapMarker.getSingleton();
    ...
    const bootstrapMarker = new BootstrapMarker({
      ...
      notes: 'Temporary emergency bootstrap'
    });
...
// Middleware for first-request bootstrap
async function firstRequestBootstrap(req, res, next) {
  ...
  try {
    console.log('🔍 First request - checking if bootstrap needed...');
    ...
    console.log('📭 Database is empty - running emergency bootstrap...');
    
    const result = await bootstrapDatabase();
    
    console.log('✅ Bootstrap completed - continuing with request');
    next();
    ...
    return res.status(500).json({
      success: false,
      message: 'Bootstrap failed - please try again later',
      error: error.message
    });
  }
}

module.exports = { firstRequestBootstrap, bootstrapDatabase };