...
const logTest = (method, endpoint, status, message, data = null) => {
    ...
    if (data && !status) console.log('   Error:', JSON.stringify(data, null, 4));
};
...
async function testHealthCheck() {
    ...
    logTest('GET', '/health', healthResult.success, 
        healthResult.success ? 'Server is healthy' : 'Health check failed', 
        healthResult.success ? healthResult.data : healthResult.error);
}
...
async function testFeeAPIs() {
    ...
    // Test Get Fees
    const feeReportResult = await makeRequest('GET', '/fee');
    logTest('GET', '/fee', feeReportResult.success, 
        feeReportResult.success ? 'Fees retrieved' : 'Fees failed', 
